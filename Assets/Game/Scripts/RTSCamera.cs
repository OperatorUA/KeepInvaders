...
public class RTSCamera : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        {
            if (Input.mousePosition.x < Screen.width * borderMoveScreen || Input.GetKey(KeyCode.A))
            {
                cameraTarget.Translate(Vector3.left * cameraSpeed * Time.deltaTime);
            }
            if (Input.mousePosition.x > Screen.width - (Screen.width * borderMoveScreen) || Input.GetKey(KeyCode.D))
            {
                cameraTarget.Translate(Vector3.right * cameraSpeed * Time.deltaTime);
            }
            if (Input.mousePosition.y < Screen.height * borderMoveScreen || Input.GetKey(KeyCode.S))
            {
                cameraTarget.Translate(Vector3.back * cameraSpeed * Time.deltaTime);
            }
            if (Input.mousePosition.y > Screen.height - (Screen.height * borderMoveScreen) || Input.GetKey(KeyCode.W))
            {
        ...
        */
        ...
        if (!lockCameraMovement)
        {
            transform.position += (-zoomSpeed * Input.GetAxis("Mouse ScrollWheel") * Vector3.up);
        }
